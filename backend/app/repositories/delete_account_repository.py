...
def create_request(db: Session, request: AccountDeletionRequest) -> AccountDeletionRequest:
    db.add(request)
    db.commit()
    db.refresh(request)
    return request

def get_active_request_by_user(db: Session, user_id: str) -> AccountDeletionRequest | None:
    return db.query(AccountDeletionRequest).filter(
        AccountDeletionRequest.user_id == user_id,
        AccountDeletionRequest.used == False,
