...
def provider_callback(
    ...
    if not code:
        return RedirectResponse(
            url=f"{config.FRONTEND_URL}/login?error=Missing authorization code"
        )
    ...
    try:
        ...
        connected_account = social_auth_repo.get_account_by_provider_id(
        ...
        if connected_account:
            ...
            if user:
                ...
                social_auth_repo.create_connected_account(
                    db, user.id, provider, provider_id, email
                )
            else:
                # 3. Create new user
                user = User(
        ...
        return RedirectResponse(
            url=f"{config.FRONTEND_URL}/login?error=Authentication failed"
        )
