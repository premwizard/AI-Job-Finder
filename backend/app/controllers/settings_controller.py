...
def request_password_change(
    db: Session, current_user: User, req: ChangePasswordRequest
) -> SuccessResponse:
    ...
    if not auth_service.validate_password_strength(req.new_password):
        raise HTTPException(
            status_code=400, detail="Password does not meet strength requirements"
        )
    ...
    existing_request = password_change_repository.get_active_request_by_user(
        db, current_user.id
    )
    if existing_request:
...
def verify_password_change(
    db: Session, current_user: User, req: VerifyPasswordChangeRequest, request: Request
) -> SuccessResponse:
    change_request = password_change_repository.get_active_request_by_user(
        db, current_user.id
    )

    if not change_request:
        raise HTTPException(
            status_code=400, detail="No active password change request found"
        )
    ...
    if change_request.attempts >= 5:
        ...
        password_change_repository.update_request(db, change_request)
        raise HTTPException(
            status_code=400, detail="Too many invalid attempts. Request a new code."
        )
    ...
    all_user_tokens = refresh_token_repository.get_active_refresh_tokens_by_user(
        db, current_user.id
    )
