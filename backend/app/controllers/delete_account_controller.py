...
def verify_deletion_otp(
    ...
    deletion_request = delete_account_repository.get_active_request_by_user(
        db, current_user.id
    )
    ...
    if not deletion_request:
        raise HTTPException(
            status_code=400,
            detail="No active deletion request found. Please start over.",
        )
    ...
    return SuccessResponse(
        success=True, message="Code verified. Please confirm to delete your account."
    )
...
def execute_account_deletion(
    ...
    if req.confirmation != "DELETE":
        raise HTTPException(
            status_code=400, detail="Confirmation text must be exactly 'DELETE'."
        )
    ...
    if deletion_request.expires_at < datetime.utcnow():
        ...
        delete_account_repository.update_request(db, deletion_request)
        raise HTTPException(
            status_code=400, detail="Verification session expired. Please start over."
        )
    ...
    logger.info(
        f"Account soft-deleted for user {current_user.id} at {current_user.deleted_at}"
    )
