...
from app.repositories import auth_repository, password_reset_repository, refresh_token_repository, email_verification_repository
...
def login_user(
    db: Session, req: LoginRequest, request: Request, response: Response
) -> TokenResponse:
...
def reset_password(db: Session, req: ResetPasswordRequest) -> SuccessResponse:
    ...
    if record.attempts >= 5:
        ...
        raise HTTPException(
            status_code=400, detail="Too many invalid attempts. Request a new link."
        )
...
def send_verification_email(db: Session, user: User) -> SuccessResponse:
    ...
    email_verification_repository.create_verification_token(
