...
class SocialAuthService:
    def get_provider_config(self, provider: AuthProvider):
        ...
        if not provider_config["client_id"] or not provider_config["client_secret"]:
            raise HTTPException(
                status_code=501,
                detail=f"{provider.value.title()} authentication is not configured.",
            )
    ...
    def get_authorization_url(
        self, provider: AuthProvider, state: str = "random_state_string"
    ) -> str:
    ...
    def get_user_info(self, provider: AuthProvider, access_token: str) -> dict:
        ...
        normalized_data = {"id": "", "email": "", "first_name": "", "last_name": ""}
        ...
        if provider == AuthProvider.google:
            ...
            name_parts = (user_data.get("name") or user_data.get("login", "")).split(
                " ", 1
            )
        ...
        if not normalized_data["id"] or not normalized_data["email"]:
            raise HTTPException(
                status_code=400,
                detail="Failed to parse required user info from provider",
            )
