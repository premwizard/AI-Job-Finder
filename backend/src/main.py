...
class AnalyticsTracker:
    ...
    def print_summary(self):
        ...
        for source, health in self.source_health.items():
            ...
            print(
                f"  {status_icon} {source.ljust(15)} | Fetched: {health['fetched']} | Status: {health['status']}"
            )
    ...
    for scraper in scrapers:
        ...
        try:
            ...
            for job in raw_jobs:
                ...
                existing_job = (
                    db.query(models.Job).filter(models.Job.job_hash == job_hash).first()
                )
                if existing_job:
            ...
            print(
                f"ERROR: Scraper {scraper.source_name} failed: {e}. Continuing with remaining scrapers..."
            )
